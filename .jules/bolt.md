...
## 2026-04-28 - Prevent N+1 Query in Course Template Lesson Loop
**Learning:** get_posts() loops that subsequently call get_post_meta() can cause N+1 database performance drains.
**Action:** Added no_found_rows => true and update_meta_cache( 'post', wp_list_pluck( $lessons, 'ID' ) ) to preload meta in a single query.
